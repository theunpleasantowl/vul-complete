BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["VUL_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["VUL_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["VUL_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["VUL_NOLINEWRAP"] != "" && ENVIRON["VUL_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
